...
function initEvent(event) {
    ...
    const eventStartTimeElement = eventElement.querySelector(
        "[data-event-start-time]"
    );
    const eventEndTimeElement = eventElement.querySelector(
        "[data-event-end-time]"
    );

    const startDate = eventTimeToDate(event.startTime);
    const endDate = eventTimeToDate(event.endTime);
...
export function eventTimeToDate(eventTime) {
    return new Date(1970, 0, 1, 0, eventTime);
}
...
export function adjustDynamicEventMaxLines(dynamicEventElement) {
    ...
    const maxTitleLines = Math.floor(
        (availableHeight - lineHeight - padding) / lineHeight
    );

    dynamicEventElement.style.setProperty(
        "--event-title-max-lines",
        maxTitleLines
    );
}
