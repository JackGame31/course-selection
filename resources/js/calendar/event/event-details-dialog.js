...
import { days } from "../utilization/date.js";
...
export function initEventDetailsDialog() {
    ...
    const deleteButtonElemenet = dialog.dialogElement.querySelector(
        "[data-event-details-delete-button]"
    );

    const editButtonElement = dialog.dialogElement.querySelector(
        "[data-event-details-edit-button]"
    );
    ...
    deleteButtonElemenet.addEventListener("click", () => {
        dialog.close().then(() => {
            deleteButtonElemenet.dispatchEvent(
                new CustomEvent("event-delete-request", {
                    detail: {
                        event: currentEvent,
                    },
                    bubbles: true,
...
function fillEventDetailsDialog(parent, event) {
    ...
    const eventDetailsTitleElement = eventDetailsElement.querySelector(
        "[data-event-details-title]"
    );
    const eventDetailsDateElement = eventDetailsElement.querySelector(
        "[data-event-details-date]"
    );
    const eventDetailsStartTimeElement = eventDetailsElement.querySelector(
        "[data-event-details-start-time]"
    );
    const eventDetailsEndTimeElement = eventDetailsElement.querySelector(
        "[data-event-details-end-time]"
    );

    eventDetailsTitleElement.textContent = event.title;
    eventDetailsDateElement.textContent = days[event.day];
    eventDetailsStartTimeElement.textContent = eventTimeFormatter.format(
        eventTimeToDate(event.startTime)
    );
    eventDetailsEndTimeElement.textContent = eventTimeFormatter.format(
        eventTimeToDate(event.endTime)
    );
