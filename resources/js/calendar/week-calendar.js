import { isTheSameDay, today, days } from "./utilization/date.js";
import {
    isEventAllDay,
    eventStartsBefore,
    eventEndsBefore,
...
import { initEventList } from "./event/event-list.js";
...
export async function initWeekCalendar(
    ...
    const calendarElement = calendarContent.querySelector(
        "[data-week-calendar]"
    );
    ...
    const weekDays = isSingleDay ? [selectedDay] : [0, 1, 2, 3, 4, 5, 6];
    for (const weekDay of weekDays) {
        const events = await eventStore.getEventsByDay(weekDay);
        ...
        if (
            deviceType === "desktop" ||
            (deviceType === "mobile" && isTheSameDay(weekDay, selectedDay))
        ) {
...
function initDayOfWeek(parent, selectedDay, weekDay, deviceType) {
    ...
    const calendarDayOfWeekButtonElement =
        calendarDayOfWeekElement.querySelector(
    ...
    calendarDayOfWeekDayElement.textContent = days[weekDay].substring(0, 3);

    if (isTheSameDay(weekDay, today().getDay())) {
        calendarDayOfWeekButtonElement.classList.add(
            "week-calendar__day-of-week-button--highlight"
        );
        if (deviceType === "mobile") {
            calendarDayOfWeekElement.classList.add(
                "bg-blue-50",
                "rounded-full",
                "px-3",
                "py-2",
    ...
    if (isTheSameDay(weekDay, selectedDay)) {
        calendarDayOfWeekButtonElement.classList.add(
...
function calculateEventsDynamicStyles(events) {
    ...
    return initialEventGroupItems.map((eventGroupItem) => {
        ...
        const bottomPercentage =
            100 - 100 * (eventGroupItem.event.endTime / 1440);
        const leftPercentage = columnWidth * eventGroupItem.columnIndex;
        const rightPercentage =
            columnWidth *
            (totalColumns -
                eventGroupItem.columnIndex -
    ...
    for (const eventGroup of eventGroups) {
        ...
        for (let i = 0; i < eventGroup.length; i += 1) {
            ...
            if (i === eventGroup.length - 1) {
                ...
                loopEventGroupItem.columnSpan =
                    nextLoopEventGroupItem.columnIndex -
                    loopEventGroupItem.columnIndex;
            }
        }
    }
